...
export interface BodySection {
  ...
  /**
   * Врезка — вынесенное крупным кеглем утверждение раздела. Нужна не для
   * украшения: длинный разбор читают по диагонали, и врезки дают тому, кто
   * листает, опорные точки, по которым видно, о чём текст. Ставится после
   * указанного по счёту абзаца, чтобы разбивать полотно, а не висеть в конце.
   */
  pullquotes?: { text: string; after: number }[]
}
...
export interface Person {
  ...
  foreign_agent?: {
    ...
    registry_url?: string
    /**
     * Имя ровно в том виде, в каком оно значится в реестре. Утверждённая форма
     * пометки называет иностранного агента поимённо, а реестр ведёт запись по
     * полному имени — оно может расходиться с тем, под которым человека знают
     * («Юрий Дудь» на странице, «Дудь Юрий Александрович» в реестре).
     * Если не задано, берётся `full_name`.
     */
    registry_name?: string
    /** Дата включения в реестр — если `listed`. */
    listed_at?: string
  }
}
