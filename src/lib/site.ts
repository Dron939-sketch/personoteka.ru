 ...
 */
/**
 * Ключ IndexNow — протокол мгновенного уведомления Яндекса и Bing о новых и
 * изменившихся страницах (Google его не поддерживает и ходит по карте сайта сам).
 *
 * Секретом не является по устройству протокола: поисковик сверяет присланный
 * ключ с файлом, который сам же скачивает с сайта, — то есть ключ обязан быть
 * общедоступным. Единственное, что он даёт, — доказательство, что заявку на
 * переобход подал владелец сайта, а не посторонний.
 *
 * Отсюда же и единственное место хранения: файл `/indexnow.txt` отдаётся
 * роутом, который берёт значение из этой константы. Разложи мы ключ по двум
 * местам — по файлу и по переменной окружения, — они рано или поздно разошлись
 * бы, и заявки стали бы отбиваться с ошибкой 422, о которой предупреждает
 * скрипт `npm run indexnow`.
 */
export const INDEXNOW_KEY = process.env.INDEXNOW_KEY ?? '39397df319379d6a8e4362bb9815ca5c'
