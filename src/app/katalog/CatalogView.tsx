...
/**
 * Тело каталога (§8.3): фильтры слева, сетка справа, алфавитный указатель сверху.
 * Один компонент на первую страницу (`/katalog/`) и на остальные
 * (`/katalog/stranica/N/`) — двум видам одного списка расходиться нельзя.
 */
const SORTS = [
...
export async function CatalogView({
  ...
  // Номер вне диапазона — не пустая страница, а 404: адрес не существует,
  // и отдавать по нему 200 значит плодить пустышки в индексе.
  const paged = paginate(sorted, page)
  if (page > paged.pages || page < 1) notFound()
  ...
  return (
    <div className="container">
      <JsonLd data={itemListJsonLd(paged.items, 'Каталог персон')} />
      ...
      <PageHeader
        ...
        meta={`Найдено: ${personsCount(paged.total)}`}
      />
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.results}>
          ...
          <Pagination base="/katalog/" page={paged.page} pages={paged.pages} total={paged.total} />
