...
/**
 * Имена, которые оканчиваются как фамилии. Без этого списка «Алина Левда»
 * читается как «Фамилия Имя» — по окончанию «-ина» — и герой получает слаг
 * с переставленным именем. Проверка по окончанию тут бессильна: различает
 * не форма слова, а знание, что так зовут, а не так называется род.
 */
const GIVEN_NAMES = new Set(
  (
    'алина ангелина арина альбина аделина валентина галина дина евгения екатерина ' +
    'ирина карина кристина марина нина полина регина элина яна ' +
    'георгий никита фома кузьма'
  ).split(' '),
)
...
function parseName(raw: string): { full: string; display: string } {
  ...
  if (
    parts.length === 2 &&
    SURNAME_ENDING.test(parts[0]) &&
    !GIVEN_NAMES.has(parts[0].toLowerCase()) &&
    !SURNAME_ENDING.test(parts[1])
  ) {
    return { full: cleaned, display: `${parts[1]} ${parts[0]}` }
  }
